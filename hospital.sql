...
('General Medicine', 1), ('Paediatrics', 2), ('Oncology', 5);
...
('Ananya Sen', '1983-08-27', 'Female', 'AB-', '9841001004', 'Pune'),
('Sunita Patil', '1955-02-14', 'Female', 'AB+', '9841001005', 'Mumbai'),
('Arjun Krishnan', '1988-09-05', 'Male', 'O-', '9841001006', 'Jaipur'),
('Geeta Sharma', '1972-03-18', 'Female', 'A-', '9841001007', 'Kochi'),
('Deepak Nambiar', '1995-12-25', 'Male', 'B-', '9841001008', 'Chandigarh'),
('Pooja Malhotra', '2001-06-08', 'Female', 'O+', '9841001009', 'Hyderabad'),
('Suresh Rajan', '1960-01-19', 'Male', 'A+', '9841001010', 'Kolkata'),
('Hari Prasad', '1945-05-03', 'Male', 'O-', '9841001011', 'Lucknow'),
('Rekha Choudhary', '1968-10-11', 'Female', 'B+', '9841001012', 'Nagpur');

INSERT INTO admissions (patient_id, doctor_id, dept_id, admit_date, discharge_date, ward, diagnosis, status) VALUES
...
SELECT a.admission_id, p.name AS patient_name, TIMESTAMPDIFF(YEAR, p.dob, a.admit_date) AS age_at_admission,
p.gender, p.blood_group, d.name AS doctor_name, d.specialization, dept.dept_name,
a.admit_date, a.discharge_date, DATEDIFF(a.discharge_date, a.admit_date) AS stay_days,
a.ward, a.diagnosis, a.status, b.total_amount AS bill_amount, b.paid_status
FROM admissions a
...
-- 3a: Patient admissions
SELECT p.name AS patient_name, p.gender, TIMESTAMPDIFF(YEAR, p.dob, CURDATE()) AS age,
d.name AS doctor_name, dept.dept_name, a.admit_date, a.discharge_date,
DATEDIFF(a.discharge_date, a.admit_date) AS length_of_stay, a.diagnosis, a.ward
FROM admissions a
JOIN patients p ON a.patient_id = p.patient_id
JOIN doctors d ON a.doctor_id = d.doctor_id
JOIN departments dept ON a.dept_id = dept.dept_id
...
-- 3b: Avg stay per department
SELECT dept.dept_name, COUNT(a.admission_id) AS total_admissions,
...
-- CTE 1a: Readmitted within 30 days
WITH discharged_patients AS (
    SELECT patient_id, admission_id, discharge_date, diagnosis
    FROM admissions
    WHERE status = 'Discharged' AND discharge_date IS NOT NULL
)
SELECT p.name AS patient_name, d1.discharge_date AS first_discharge,
d1.diagnosis AS first_diagnosis, d2.admit_date AS readmission_date,
DATEDIFF(d2.admit_date, d1.discharge_date) AS days_between
FROM discharged_patients d1
JOIN admissions d2 ON d1.patient_id = d2.patient_id 
...
-- Window 2a: Running Total
SELECT YEAR(a.admit_date) AS yr, MONTHNAME(a.admit_date) AS month_name, MONTH(a.admit_date) AS month_num,
SUM(b.total_amount) AS monthly_revenue,
SUM(SUM(b.total_amount)) OVER (ORDER BY YEAR(a.admit_date), MONTH(a.admit_date)) AS running_total
FROM billing b
...
BEGIN
    SELECT 'MONTHLY HOSPITAL REPORT' AS report_title, p_year AS report_year, p_month AS report_month;

    SELECT COUNT(a.admission_id) AS total_admissions, SUM(b.total_amount) AS total_revenue,
    ...
    WHERE YEAR(a.admit_date) = p_year AND MONTH(a.admit_date) = p_month;

    SELECT dept.dept_name, COUNT(a.admission_id) AS admissions, SUM(b.total_amount) AS dept_revenue
    FROM admissions a
    JOIN departments dept ON a.dept_id = dept.dept_id
    JOIN billing b ON a.admission_id = b.admission_id
    WHERE YEAR(a.admit_date) = p_year AND MONTH(a.admit_date) = p_month
    ...
    SELECT d.name AS doctor_name, COUNT(a.admission_id) AS patients_treated
    FROM admissions a
    JOIN doctors d ON a.doctor_id = d.doctor_id
    WHERE YEAR(a.admit_date) = p_year AND MONTH(a.admit_date) = p_month
...
-- January 2023 ki hospital report dekhne ke liye:
CALL GenerateMonthlyReport(2023, 1);

-- May 2023 ki hospital report dekhne ke liye:
